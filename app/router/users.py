...
from app.schemas.schema import RequestUser, Response, UserSchema, RequestUpdateUser, UpdateUser
...
@router.patch("/update", response_model=UpdateUser, status_code=201)
async def update_user(request: RequestUpdateUser,
                      data: AsyncSession = Depends(get_db),
                      token: str = Depends(token_auth_scheme)) -> User:
    _user = await crud.UserCrud(data).update_user(user_email=await get_user_or_auth(db=data, token=token),
...
async def delete_user(data: AsyncSession = Depends(get_db), token: str = Depends(token_auth_scheme)):
    try:
        await crud.UserCrud(data).remove_user(user_email=await get_user_or_auth(db=data, token=token))
    except:
        raise HTTPException(status_code=404, detail="User id not found")
