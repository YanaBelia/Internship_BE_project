...
async def create_user(request: RequestUser, data: Session = Depends(get_db)) -> UserSchema:
    _user = await crud.create_user(data, user=request.parameter)
    if not _user:
...
async def get_users(skip: int = 0, limit: int = 100, data: Session = Depends(get_db)) -> User:
    _user = await crud.get_user(data, skip, limit)
    if not _user:
...
async def get_by_id(id: int, data: Session = Depends(get_db)) -> UserSchema:
    _user = await crud.get_user_by_id(data, id)
    if not _user:
...
async def update_user(request: RequestUser, data: Session = Depends(get_db)) -> UserSchema:
    _user = await crud.update_user(data, user_id=request.parameter.id,
...
async def get_by_id(id: int, data: Session = Depends(get_db)) -> UserSchema:
    _user = await crud.get_user_by_id(data, id)
    if not _user:
        raise HTTPException(status_code=404, detail="User id not found")  # working)
    return _user
